        ...
        foreach ($records as $record) {
            ...
            if ($file->getClientOriginalExtension() === 'csv') {
                ...
                Account::create([
                    ...
                    'tanggal_terdaftar' => $record[6] ?? null,
                    'tanggal_aktivasi' => $record[7] ?? null,
                    'status_pengusaha_kena_pejak' => $record[8] ?? null,
                    'tanggal_pengukuhan_pkp' => $record[9] ?? null,
                    'kantor_wilayah_direktorat_jenderal_pajak' => $record[10] ?? null,
                    'kantor_pelayanan_pajak' => $record[11] ?? null,
                    'seksi_pengawasan' => $record[12] ?? null,
                    'tanggal_pembaruan_profil_terakhir' => $record[13] ?? null,
                    'alamat_utama' => $record[14] ?? null,
                    'nomor_handphone' => $record[15] ?? null,
                    'email' => $record[16] ?? null,
                    'kode_klasifikasi_lapangan_usaha' => $record[17] ?? null,
                    'deskripsi_klasifikasi_lapangan_usaha' => $record[18] ?? null,
                ]);
            }
        }
    }
